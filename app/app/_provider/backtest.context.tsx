    ...
    const handleComplete = async (data: any) => {
      ...
      try {
        ...
        const transformedResult: BacktestResult = {
            ...
            strategyName: data.summary?.strategyName || fullResult.strategy_name || 'Unknown',
            leverageApplied: data.summary?.leverageApplied || fullResult.leverage_applied || 1,
            totalReturn: data.summary?.totalReturn || 0,
            sharpeRatio: data.summary?.sharpeRatio || fullResult.performance?.sharpe || 0,
            maxDrawdown: data.summary?.maxDrawdown || fullResult.performance?.max_drawdown || 0,
            totalTrades: data.summary?.totalTrades || fullResult.performance?.trades || 0,
            winRate: data.summary?.winRate || fullResult.performance?.win_rate || 0,
            finalBalance: data.summary?.finalBalance || fullResult.levered_performance?.final_usdt_levered || 0,
